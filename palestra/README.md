# A apresentação da palestra

**Sua equipe de IA cabe num arquivo de texto**
Centro Universitário do Triângulo (UNITRI), Uberlândia, 25 de agosto de 2026. Matheus Dariva.

📄 **[deck-palestra-unitri-2026-08-25.pdf](deck-palestra-unitri-2026-08-25.pdf)**

O deck acompanha uma linha do tempo de maio a agosto de 2026: como a operação saía de conversas
que recomeçavam do zero, o desenho do sistema de papéis, a primeira instalação para outra pessoa
e o que o método aprendeu no caminho.

O caminho prático apresentado no final da palestra está neste repositório: comece pelo
[GUIA-COMPLETO.md](../GUIA-COMPLETO.md) e pela pasta [sistema-inicial/](../sistema-inicial/).

## Sobre os números do deck

Dois números do slide "A prova real" merecem contexto. *"Cerca de um terço do meu método
atravessou"* é **estimativa do autor**, não medição: a parte contável são seis pares de
regra transferida×cortada no log daquela implantação. E *"uma gerência e três frentes reais"*
é a contagem daquela instalação específica, registrada no log de decisões dela.

## Direitos de imagem

As imagens do filme *Como se fosse a primeira vez* (Sony Pictures, 2004) que aparecem no deck
são citadas como referência, com crédito, para fins educacionais. Elas pertencem
aos seus detentores e **não são cobertas** pela licença [CC BY 4.0](../LICENSE) deste repositório.
Todo o restante do material é próprio e segue a licença do repositório.
